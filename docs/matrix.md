...
LED matrix driving relies on a phenomenon called "[persistence of vision](https://en.wikipedia.org/wiki/Persistence_of_vision)". There is a really nice [animation](https://en.wikipedia.org/wiki/Dot_matrix#LED_matrix) on Wikipedia that I feel obliged to attach:

<p><img alt="An animated picture of LED matrix" src="img/matrix_animation.gif" width=30% height=30%/></p>

The LED matrix in this project is driven left to right, just like in the picture! The algorithm is simple: enable rows (LED anodes) and then enable one column (LED cathodes). Once the time comes for another column, switch the old column off, update rows and enable the new column. If you do it fast enough, you'll get an illusion of all columns working simultaneously. But the devil is, of course, in the details!
...
- ATmega32 has only four PWM channels, and one is already used to drive the buzzer. Not enough for every row!
- Even if there were enough channels (or we applied PWM to LED power supply), combining them with matrix-driving is tricky: the PWM and column switch frequencies might alias in a bad way, producing nasty flickering. I've learned that the hard way!
...
- Each full display refresh (that's when every column gets scanned and we return to the first one) is assigned an incrementing number with an overflow value.
...
A very common issue for DIY LED matrix displays is ghosting. The most common cause is software defects: it can happen when LED rows are reconfigured without disabling the column first. If the display is scanned left-to-right, you'll see ghosts to the **left side** of lit pixels.

But there can also be another cause for ghosting: **parasitic capacitance**. Turns out, that LEDs are also capacitors and can store a tiny charge. On fast refresh speeds and small brightness levels this can result in ghosting as well! If display is scanned left-to-right, you'll see a ghosting trail to the **right side** of lit pixels.

The second issue is way nastier, since it can't be fully defeated in software. It can be somewhat reduced by several workarounds, though:
...
- Use a push-pull driver for LED anodes (or add some pull-down resistors), this will help discharge the capacitance in LEDs.
- Use MOSFETs with low parasitic capacitance for LED cathodes (or use BJTs instead), so you won't have extra capacitance on the low side.
